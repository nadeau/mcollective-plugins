...
The ACTION can be one of the following:
    ...
    unlink  - removed puppet lock file
    END_OF_USAGE
  ...
  def post_option_parser(configuration)
    if ARGV.length >= 1
      ...
      unless configuration[:command].match(/^(enable|disable|runonce|runall|status|summary|count|unlink)$/)
        raise "Action must be enable, disable, runonce, runonce, runall, status, summary, count or unlink"
      end
    else
